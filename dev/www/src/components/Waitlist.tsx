...
export const WaitList = () => {
  ...
  return (
    <form
      className="gap-6 mt-14 mb-7 xl:flex space-y-4 xl:space-y-0"
      onSubmit={async (ev) => {
        ev.preventDefault();
        ...
        const result = await fetch('/waitlist.json', {
    ...
    >
      <input
        type="email"
        className="border rounded-md bg-transparent p-3 xl:w-1/3 w-full"
        placeholder="Waitlist (Email)"
        disabled={state === 'loading' || state === 'done'}
        required
      />
      <button
        className={`py-3 px-8 bg-gradient-to-tr from-[#defff8] to-[#b1fee9] text-gray-900 gap-4 rounded-md xl:w-auto w-full ${
